  ...
  def train(self, epoch, data_loader, optimizer, current_lr=0.0,
      ...
      if global_args.iter_mode == True:
        ...
        if self.iters % global_args.evaluation_freq_iter == 0:
          if 'loss_rec' not in output_dict['loss']:
            ...
            with torch.no_grad():
              self.best_res = evaluator.evaluate(test_loader, step=epoch, tfLogger=eval_tfLogger)
          else:
            with torch.no_grad():
    ...
    if epoch % global_args.evaluation_freq_epoch == 0:
      if 'loss_rec' not in output_dict['loss']:
        is_best = True
        # self.best_res is alwarys equal to 1.0
        self.best_res = evaluator.evaluate(test_loader, step=epoch, tfLogger=eval_tfLogger, dataset=test_dataset)
      else:
        res,pred_list, score_list, targ_list = evaluator.evaluate(test_loader, step=epoch, tfLogger=eval_tfLogger)
