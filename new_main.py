...
def get_data(data_dir, height, width, batch_size, workers, is_train, keep_ratio, alphabets, trans=None, randomSampler=False,num_samples=math.inf):
  if isinstance(data_dir, list):
    dataset_list = []
    for data_dir_ in data_dir:
      dataset_list.append(lmdbDataset(alphabets, data_dir_,transform=trans,num_samples=num_samples))
    dataset = ConcatDataset(dataset_list)
  else:
    dataset = lmdbDataset(alphabets, data_dir,transform=trans,num_samples=num_samples)
